...
public class AudioRecorderService extends Service {
	...
	public void onCreate() {
		...
		try {
			...
			mWl = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, AudioRecorderService.class.getName());
			mWl.acquire();

			HandlerThread thread = new HandlerThread("AudioRecorderHandler", Process.THREAD_PRIORITY_BACKGROUND);
			thread.start();
			...
			mWavAudioRecorder = new RehearsalAudioRecorder(AudioSource.MIC, 8000, AudioFormat.CHANNEL_IN_MONO,
			...
			registerReceiver(mIncomingCallDetector, new IntentFilter("android.intent.action.PHONE_STATE"));
			registerReceiver(mOutgoingCallDetector, new IntentFilter(Intent.ACTION_NEW_OUTGOING_CALL));
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		PendingIntent contentIntent = PendingIntent.getActivity(this, 0, new Intent(this,
				SensorlabRecorderActivity.class), 0);
	...
	private String getFileOnSD() {
		...
		String filename = String.format("%s_%s.wav", imei, sdf.format(new Date()));

		File root = new File(Environment.getExternalStorageDirectory(), AUDIO_RECORDING_DIR);
		if (!root.exists()) {
			root.mkdir();
		}
	...
	private void startRecoding(boolean startTimer) {
		if (mWavAudioRecorder.getState() != RehearsalAudioRecorder.State.RECORDING) {
			if (startTimer) {
				mTimer = new Timer();
				mTimer.schedule(new RollWaveFile(), WAV_CHUNK_LENGTH_MS, WAV_CHUNK_LENGTH_MS);
			}
